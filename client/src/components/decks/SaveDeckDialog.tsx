    ...
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal-panel bg-md-surface border border-md-border rounded-xl shadow-surface-lg w-full max-w-sm p-4" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex gap-2 justify-end">
          <button onClick={onClose} className="press text-sm px-3 py-2 rounded-lg bg-md-surfaceHover hover:bg-md-borderLight">Cancel</button>
          <button
            disabled={busy || !name.trim()}
            onClick={handleSave}
            className="press text-sm font-semibold px-3 py-2 rounded-lg bg-md-blue text-white hover:bg-md-blueLight disabled:opacity-50"
          >
            {busy ? 'Saving…' : 'Save'}
          </button>
        </div>
